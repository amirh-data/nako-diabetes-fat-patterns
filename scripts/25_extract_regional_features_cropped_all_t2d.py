...
def crop_to_mask_bbox(fat, seg, margin=10):
    mask = seg > 0
    ...
    zmax = min(zmax + margin, fat.shape[0] - 1)
    ymax = min(ymax + margin, fat.shape[1] - 1)
    xmax = min(xmax + margin, fat.shape[2] - 1)

    fat_crop = fat[zmin:zmax + 1, ymin:ymax + 1, xmin:xmax + 1]
    seg_crop = seg[zmin:zmax + 1, ymin:ymax + 1, xmin:xmax + 1]

    return fat_crop, seg_crop
...
def main():
    ...
    for i, row in df.iterrows():
        ...
        fat, seg = crop_to_mask_bbox(fat, seg, margin=10)
